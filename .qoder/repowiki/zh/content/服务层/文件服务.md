...
## 更新摘要
**变更内容**
- 新增 ImageFileCollection 类和对应的 GetImageFiles 重载方法，支持单次遍历目录并缓存结果
- 优化批量插入性能，避免重复扫描目录导致的性能问题
- 更新 ProgressService 中的文件遍历逻辑，使用新的缓存机制
...
- GetImageFiles（传统重载）：
  ...
  - 排序：对文件名进行自然排序后返回。
- GetImageFiles（新重载）：
  - **新增功能**：支持单次遍历目录并缓存结果，避免重复扫描。
  - 参数：includeRootImages 和 includeSubFolderImages 控制包含范围。
  - 返回：ImageFileCollection 对象，包含 RootFiles、SubfolderFiles 和 TotalCount。
  - 性能优势：一次遍历即可获取所有需要的信息，适合大批量文件处理。
- GetRootImageFiles：
...
- 使用建议：
  ...
  - 大批量文件时优先使用新的 ImageFileCollection 重载。
...
### ImageFileCollection 类：缓存遍历结果
- **新增功能**：专门用于缓存目录遍历结果的类。
- 属性：
  - RootFiles：根目录中的图片文件数组。
  - SubfolderFiles：字典，键为子文件夹路径，值为该文件夹中的图片文件数组。
  - TotalCount：所有图片文件的总数。
- 设计目的：
  - 避免重复扫描同一目录多次导致的性能问题。
  - 支持分层次的数据访问，便于批量插入时的组织。
- 使用场景：
  - 批量插入时一次性获取所有需要的文件信息。
  - 支持增量处理，逐步释放不需要的引用以节省内存。
...
- 文件枚举：
  ...
  - **新增优化**：使用 ImageFileCollection 重载可避免重复扫描，显著提升大批量文件处理性能。
- 进度与内存：
  - 进度服务根据文件总数动态调整刷新与内存清理频率，减少 UI 卡顿。
  - 批量插入采用快速插入策略（InsertImageFast）降低内存占用。
  - **新增优化**：ImageFileCollection 支持分层次数据访问，便于逐步释放内存。
...
- 批量插入卡顿：
  ...
  - **新增建议**：使用新的 ImageFileCollection 重载处理大批量文件，避免重复扫描导致的性能问题。
...
## 结论
FileService 以简洁的静态接口提供了文件夹选择、图片文件选择、文件验证、文件列表获取与自然排序等核心能力，配合 InsertPhotosForm 与 ProgressService 形成了完整的批量插图工作流。其设计遵循单一职责原则，易于维护与扩展；同时通过自然排序与性能优化策略提升了用户体验。**新增的 ImageFileCollection 类和对应的重载方法进一步优化了大批量文件处理的性能，避免了重复扫描目录的问题。**
...
- 性能优化：
  ...
  - **新增建议**：使用 ImageFileCollection 重载处理大批量文件，避免重复扫描目录。
- 数据访问模式：
  - 使用新的 ImageFileCollection 可以一次性获取所有需要的文件信息，然后按需访问 RootFiles 或 SubfolderFiles。
