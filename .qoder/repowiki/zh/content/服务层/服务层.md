...
- TableService：Word表格处理（单元格校验、插入图片、自动编号、布局管理、智能字段管理、增量差异处理）
- ProgressService：批量插图进度监控（取消控制、性能优化、状态通知）
- ImageService：图片插入与尺寸调整（厘米/磅换算、快速插入、批量调整）
- ConfigService：配置持久化（文档自定义属性与注册表）
- EDF_DataFillerService 与 EDF_TemplateDetector：Excel数据填充与模板检测（智能模板检测、错误处理增强）
...
- TableService：提供表格校验、单元格适合性判断、行/列调整、固定列宽、标题行/描述行创建、自动编号清理与应用、智能字段管理、增量差异处理、文本清理。
...
- **智能字段管理**：支持Word SEQ域的智能管理，包括编号状态扫描、增量差异处理、域值更新。
- **增量差异处理**：通过预扫描行图片状态、位置映射和域分析，实现高效的增量编号更新。
...
- 编号：ClearTableNumbering(tbl, startRow=1) -> 原对齐方式；AddNumberingToDescriptionRows(tbl, doc, startRow, alignment, needAutoNumbering)；CalculateNextSequenceNumber(tbl, startRow)；InsertSeqField(tbl, rowIdx, colIdx, alignment, ref isFirstSeqField, startNumber)
- **智能字段**：RefreshTableNumbering(tbl, doc, alignment, progressCallback) - 智能增量差异处理

实现要点
- 对Word COM对象的访问采用try/catch兜底，避免单点异常中断流程。
- 自动编号清理支持两种场景：列表模板编号与纯文本序号，确保可复用。
- 对齐方式与编号起始值通过历史行推断，保证连续性。
- **智能字段管理**：采用两阶段扫描（图片状态扫描和域分析）+ 增量差异处理，显著提升性能。

**更新** 增量差异处理功能得到显著增强，包括智能字段管理、位置映射优化、域更新策略改进
...
- 表格服务
  ...
  - 固定列宽减少Word自动调整列宽的开销。
  - **智能字段管理优化**：通过两阶段扫描（图片状态扫描和域分析）+ 增量差异处理，显著提升编号处理性能。
  - **位置映射优化**：使用二分查找算法快速定位域所在的行，避免昂贵的COM调用。
  - **域更新策略**：智能判断是否需要Fields.Update()，避免不必要的全表更新。
- 图片服务
...
- 表格服务
  ...
  - **智能字段异常**：检查域扫描和位置映射逻辑，确认图片状态检测的准确性。
  - **编号功能异常**：检查正则表达式匹配、Range对象状态和对齐方式检测逻辑。
- 图片服务
...
**更新** TableService的智能字段管理和增量差异处理功能经过全面优化，包括两阶段扫描策略、位置映射优化、域更新智能判断和错误处理增强，显著提升了表格处理服务的整体性能和可靠性。
...
- 批量插图（文件夹）
  ...
  - 参数：folderPath、minHeight（厘米输入，内部转换为磅）、needDescription、useFileNameAsDescription、includeRootImages、includeSubFolderImages、needAutoNumbering、numberAlignment（1=靠左，2=居中，3=居右）
...
- 性能优化
  ...
  - **更新**：智能字段管理优化：采用两阶段扫描策略、位置映射优化和智能域更新策略。
- 错误处理
