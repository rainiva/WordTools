...
## 更新摘要
**变更内容**
- 新增三级垃圾回收策略章节，详细说明内存水位监控和优化的刷新间隔
- 更新性能优化章节，包含复杂的内存管理机制
- 增强内存清理策略的实现细节
- 更新进度数据管理章节，包含新的内存清理间隔配置
...
**更新** 本版本特别关注ProgressService实现的复杂垃圾回收策略，包括三级GC系统、内存水位监控和优化的刷新间隔机制。
...
## 核心组件
- ProgressService：负责批量图片插入的进度管理、性能优化、取消控制与状态栏更新，实现复杂的三级垃圾回收策略。
...
### ProgressService 类
- 角色定位：管理批量插图进度，提供性能优化与取消控制，实现复杂的三级垃圾回收策略。
- 关键职责：
  ...
  - **新增** 三级垃圾回收：实现内存水位监控、分级GC策略和优化的刷新间隔。
- 数据结构与算法：
  - 刷新间隔动态调整：根据总文件数映射不同刷新频率，平衡UI响应与性能。
  - **新增** 内存清理策略：实现三级GC系统，包括内存水位监控和定期全代回收。
...
**更新** ProgressService实现了复杂的三级垃圾回收策略，包括内存水位监控、分级GC策略和优化的刷新间隔配置。
...
**更新** ProgressService实现了复杂的三级垃圾回收策略，这是本次更新的核心内容。

- **内存水位监控**：当GC.GetTotalMemory(false)超过500MB时，强制执行全代回收（GC.Collect(2)），防止内存泄漏导致的性能下降。
- **分级GC策略**：
  - 第0代回收：常规快速回收，使用GC.Collect(0)，适用于日常的小规模内存清理。
  - 全代回收：定期执行（由_fullGcInterval控制），防止大对象堆膨胀和内存碎片化。
  - 强制全代回收：当内存水位超过阈值时立即执行，确保系统稳定性。
- **优化的刷新间隔**：通过_getOptimizedRefreshInterval方法根据文件总数动态调整刷新频率，平衡UI响应与性能开销。
- **内存清理周期**：_memoryCleanInterval设置为_refreshInterval的10倍，确保内存清理不会过于频繁影响性能。
- **全代回收间隔**：_fullGcInterval设置为_memoryCleanInterval的5倍，提供长期的内存维护策略。

```mermaid
flowchart TD
Start(["开始批量处理"]) --> InitParams["初始化参数<br/>刷新间隔=优化值<br/>内存清理间隔=刷新间隔×10<br/>全代回收间隔=内存清理间隔×5"]
InitParams --> Loop["主处理循环"]
Loop --> CheckCancel["检查取消标志"]
CheckCancel --> |已取消| Cleanup["清理并退出"]
CheckCancel --> |未取消| UpdateStatus["按刷新间隔更新状态栏"]
UpdateStatus --> InsertImage["插入图片"]
InsertImage --> CheckMemory["检查内存清理条件"]
CheckMemory --> |达到清理间隔| MemoryCleanup["执行三级GC策略"]
CheckMemory --> |未达到间隔| NextIter["继续下一次迭代"]
MemoryCleanup --> MemoryWatermark{"内存水位>500MB?"}
MemoryWatermark --> |是| FullGC["执行全代回收"]
MemoryWatermark --> |否| CheckFullGC{"达到全代回收间隔?"}
CheckFullGC --> |是| FullGC
CheckFullGC --> |否| Gen0GC["执行第0代回收"]
FullGC --> NextIter
Gen0GC --> NextIter
NextIter --> Loop
Cleanup --> End(["结束"])
...
**图表来源**
- [ProgressService.cs:119-124](file://WordTools/Services/ProgressService.cs#L119-L124)
- [ProgressService.cs:129-157](file://WordTools/Services/ProgressService.cs#L129-L157)
- [ProgressService.cs:226-231](file://WordTools/Services/ProgressService.cs#L226-L231)
...
- **新增** 三级垃圾回收：实现内存水位监控、分级GC策略和优化的刷新间隔，缓解大批量处理的内存压力。
...
**更新** 新增三级垃圾回收策略的性能考量，包括内存水位监控和分级GC策略对性能的影响。
...
**更新** 新增内存相关问题的故障排除指南，包括内存水位监控和三级GC策略的问题诊断。
...
**更新** 本次更新特别强调了ProgressService实现的复杂垃圾回收策略，包括三级GC系统、内存水位监控和优化的刷新间隔，这些特性显著提升了大批量处理场景下的内存管理和系统稳定性。
...
**更新** 新增内存监控和调试技巧，帮助开发者理解和优化ProgressService的内存管理策略。