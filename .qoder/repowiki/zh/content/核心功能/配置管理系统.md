...
## 更新摘要
**变更内容**
- 新增自动编号配置支持，包括全局用户偏好和文档级配置
- 改进配置迁移和兼容性处理，特别是文件范围配置的默认值逻辑
- 增强用户偏好的持久化机制，确保全局配置的一致性
- 优化配置加载和保存的实现细节，提升系统稳定性
...
**更新** 本版本增强了配置管理功能，新增自动编号配置支持和改进的配置迁移机制，提升了系统的灵活性和向后兼容性。
...
**更新** 新增了自动编号配置和改进的配置迁移机制，提升了系统的灵活性和向后兼容性。
...
**更新** 新增了自动编号配置的全局存储机制，确保用户偏好在不同文档间的一致性。
...
**更新** 改进了文件范围配置的默认值处理，确保新旧版本的向后兼容性。
...
- 包含范围：包括"是否包含根目录图片"和"是否包含子目录图片"，均为布尔值，支持文档级与全局级存储。**更新** 改进了默认值处理，空值或True时默认为true，提升用户体验。
- 自动编号设置：布尔值，仅支持全局级存储（注册表）。**新增** 支持全局用户偏好的自动编号配置。
- 对齐方式：整数（1=靠左，2=居中），默认值为2，仅支持全局级存储（注册表）。**更新** 改进了配置迁移机制，确保新旧版本的兼容性。
...
**更新** 新增了自动编号配置的加载和保存机制，确保用户偏好的正确持久化。
...
### 配置迁移和兼容性处理
- 版本升级兼容：新增配置项时，应提供默认值，确保旧版本文档或未设置的用户能正常工作。**更新** 改进了文件范围配置的默认值处理，确保向后兼容性。
- 数据类型迁移：对于数值型配置，使用 Parse 并限定有效范围（如编号对齐仅接受1或2），无效值回退到默认值。**更新** 增强了配置迁移机制，提升系统稳定性。
- 空值处理：统一使用"__EMPTY__"标记区分空值，避免 Word COM 对空字符串的不兼容问题。
- 注册表路径：固定在 HKCU\Software\WordTools，避免多用户环境下的冲突。

**更新** 新增了自动编号配置的迁移支持，确保用户偏好的正确继承。
...
- **自动编号配置异常**：**新增** 检查全局配置是否正确保存到注册表，确认用户偏好的一致性。
...
**更新** 本版本的改进包括新增自动编号配置支持、改进的配置迁移机制和增强的用户偏好持久化，进一步提升了系统的灵活性和用户体验。
