...
- [ProgressService.cs:1-804](file://WordTools/Services/ProgressService.cs#L1-L804)
...
- [ProgressService.cs:15-804](file://WordTools/Services/ProgressService.cs#L15-L804)
...
- [TableService.cs:13-1713](file://WordTools/Services/TableService.cs#L13-L1713)
...
- [ProgressService.cs:163-413](file://WordTools/Services/ProgressService.cs#L163-L413)
- [ProgressService.cs:419-576](file://WordTools/Services/ProgressService.cs#L419-L576)
...
**图表来源**
- [TableService.cs:476-1444](file://WordTools/Services/TableService.cs#L476-L1444)
- [TableService.cs:1606-1675](file://WordTools/Services/TableService.cs#L1606-L1675)
- [ProgressService.cs:580-766](file://WordTools/Services/ProgressService.cs#L580-L766)
...
- [ImageService.cs:73-180](file://WordTools/Services/ImageService.cs#L73-L180)
- [ImageService.cs:195-361](file://WordTools/Services/ImageService.cs#L195-L361)
...
- **内存管理优化**：ProgressService中优化的垃圾回收策略，包括分级GC和定期全代回收
- **纯文本编号系统**：TableService中新的纯文本编号方式，避免SEQ域开销，提升编号性能

**更新** 重大性能优化：BatchAddRows批处理策略从100行/批提升至200行/批，大幅减少COM调用次数

**章节来源**
- [ProgressService.cs:75-114](file://WordTools/Services/ProgressService.cs#L75-L114)
- [ImageService.cs:250-361](file://WordTools/Services/ImageService.cs#L250-L361)
...
**内存管理优化**：
- **分级GC策略**：根据处理进度动态调整GC级别，减少内存碎片
- **定期全代回收**：每50次处理后进行全代回收，防止大对象堆膨胀
- **内存水位监控**：超过500MB时强制全代回收，确保内存使用效率

**纯文本编号系统优化**：
- **避免SEQ域开销**：使用纯文本编号替代Word SEQ域，减少DOM操作
- **每列独立递增**：支持2列表格的独立编号，提升灵活性
- **快速路径优化**：使用正则表达式快速匹配和更新编号

**章节来源**
- [ImageService.cs:278-314](file://WordTools/Services/ImageService.cs#L278-L314)
...
- [ImageService.cs:43-60](file://WordTools/Services/ImageService.cs#L43-L60)
- [TableService.cs:1321-1444](file://WordTools/Services/TableService.cs#L1321-L1444)
- [ProgressService.cs:75-114](file://WordTools/Services/ProgressService.cs#L75-L114)

## 结论
WordTools的图片批量插入功能通过窗体层与服务层的清晰分工，实现了从文件夹或文件选择到表格插入的完整自动化流程。DPI缩放适配、性能优化、进度监控与自动编号系统共同保障了用户体验与稳定性。**最新的BatchAddRows性能优化**将批处理效率从100行/批提升至200行/批，大幅减少了COM调用次数，显著提升了大表格操作性能。**纯文本编号系统的引入**进一步优化了编号性能，避免了传统SEQ域的开销。开发者可根据实际需求调整参数与策略，以获得最佳的批量处理效果。
...
**章节来源**
- [ConfigService.cs:14-467](file://WordTools/Services/ConfigService.cs#L14-L467)

### 性能优化最佳实践
- **大表格处理**：确保使用最新版本的BatchAddRows优化（200行/批）
- **预分配策略**：合理估算图片数量，使用PreAllocateRows提前分配行数
- **批量操作**：优先使用批量插入而非逐行操作
- **内存管理**：监控内存使用，避免超过500MB触发全代回收
- **编号系统选择**：使用纯文本编号系统替代SEQ域，提升性能
