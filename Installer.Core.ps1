...
function Ensure-EvidenceMarkdownTable {
    ...
    if (-not (Test-Path -LiteralPath $fullPath)) {
        $initialText = @(
            ...
            "- Do not change the current Word x64 live registration path during the probe phase.",
            ...
            "| WPS | x86 | Experimental | experimental ui passed | Historical UI failure recorded on 2026-05-24; AddinsWl experiment confirmed a real UI entry on 2026-05-26, but P0 remains pending |",
...
function Get-ValidationStageForProbeResult {
    ...
    $experimentalHosts = @($ProbeResult.SupportSummary.ExperimentalHosts)
    ...
    if ($experimentalHosts.Count -gt 0) {
        return "experimental ui passed"
    }
...
function Get-WpsReconData {
    ...
    function Get-AddonPoolReconIndex {
        param(
            [Parameter(Mandatory = $true)]
            [string]$RootPath
        )

        $commandDatabaseFiles = [System.Collections.Generic.List[string]]::new()
        $metadataFilesByName = @{
            "runinfo.json"         = [System.Collections.Generic.List[string]]::new()
            "config.json"          = [System.Collections.Generic.List[string]]::new()
            "run.ini"              = [System.Collections.Generic.List[string]]::new()
            "preload-file.json"    = [System.Collections.Generic.List[string]]::new()
            "plugin-provider.json" = [System.Collections.Generic.List[string]]::new()
            "__attr.plg"           = [System.Collections.Generic.List[string]]::new()
        }
        $suspiciousModuleDirectoryPrefixesByName = @{
            "kpluginmanager.dll" = "kpluginmanager_"
            "kvbarunner.dll"     = "kvbarunner_"
            "kwpsremixsdksrv.dll" = "kwpsremixsdksrv_"
            "wpsr.dll"           = "wpsioplugin_"
            "wppr.dll"           = "wpsioplugin_"
            "et10rw.dll"         = "wpsioplugin_"
        }
        $suspiciousModulePathsByName = @{}

        if ([string]::IsNullOrWhiteSpace($RootPath) -or -not (Test-Path -LiteralPath $RootPath -PathType Container)) {
            return [pscustomobject]@{
                CommandDatabaseFiles       = @()
                MetadataFilesByName        = $metadataFilesByName
                SuspiciousModulePathsByName = $suspiciousModulePathsByName
            }
        }

        foreach ($filePath in [System.IO.Directory]::EnumerateFiles($RootPath, "*", [System.IO.SearchOption]::AllDirectories)) {
            $fileName = [System.IO.Path]::GetFileName($filePath)
            if ($metadataFilesByName.ContainsKey($fileName)) {
                $metadataFilesByName[$fileName].Add($filePath)
            }

            if ($filePath -match "\\kcmddb_[^\\]+\\db\\personal_cn\\(wps|wpp|et|pdf)\.db$") {
                $commandDatabaseFiles.Add($filePath)
            }

            if ($suspiciousModuleDirectoryPrefixesByName.ContainsKey($fileName) -and -not $suspiciousModulePathsByName.ContainsKey($fileName)) {
                $parentDirectoryName = [System.IO.Path]::GetFileName([System.IO.Path]::GetDirectoryName($filePath))
                $expectedPrefix = [string]$suspiciousModuleDirectoryPrefixesByName[$fileName]
                if (-not [string]::IsNullOrWhiteSpace($parentDirectoryName) -and $parentDirectoryName.StartsWith($expectedPrefix, [System.StringComparison]::OrdinalIgnoreCase)) {
                    $suspiciousModulePathsByName[$fileName] = $filePath
                }
            }
        }

        return [pscustomobject]@{
            CommandDatabaseFiles       = @($commandDatabaseFiles.ToArray())
            MetadataFilesByName        = $metadataFilesByName
            SuspiciousModulePathsByName = $suspiciousModulePathsByName
        }
    }
    ...
    function Get-NamedDllExports {
        ...
        if ($namedDllExportsCache.ContainsKey($Path)) {
            return @($namedDllExportsCache[$Path])
        }

        try {
            $bytes = [System.IO.File]::ReadAllBytes($Path)
            if ([System.Text.Encoding]::ASCII.GetString($bytes, 0, 2) -ne "MZ") {
                $namedDllExportsCache[$Path] = @()
                return @()
            }
            ...
            $namedExportArray = @($exports.ToArray())
            $namedDllExportsCache[$Path] = $namedExportArray
            return @($namedExportArray)
        }
        catch {
            $namedDllExportsCache[$Path] = @()
            return @()
    ...
    $addonPoolReconIndex = Get-AddonPoolReconIndex -RootPath $addonPoolRoot
                    ...
                    if ($null -ne $poolPackageMatch) {
                        ...
                        if ($poolPackageHasRunInfoJson) {
                            try {
                                ...
                                foreach ($mappedApp in $mappedRunInfoRoot.PSObject.Properties) {
                                    $mappedEntryDll = [string]$mappedApp.Value.entryDll
                                    if ([string]::IsNullOrWhiteSpace($mappedEntryDll)) {
                                        $mappedEntryDll = [string]$mappedApp.Value.launcherDll
                                    }

                                    $mappedEntryPoint = [string]$mappedApp.Value.entryPoint
                                    $mappedLauncherType = [string]$mappedApp.Value.launcherType
                                    if ([string]::IsNullOrWhiteSpace($mappedEntryDll) -or [string]::IsNullOrWhiteSpace($mappedEntryPoint) -or [string]::IsNullOrWhiteSpace($mappedLauncherType)) {
                                        continue
                                    }

                                    $poolPackageRunInfoAppId = [string]$mappedApp.Name
                                    $poolPackageEntryDll = $mappedEntryDll
                                    $poolPackageEntryPoint = $mappedEntryPoint
                                    $poolPackageLauncherType = $mappedLauncherType
                                    break
                                }
                            }
                            catch {
    ...
    if (Test-Path -LiteralPath $setupPluginAuthInfoPath -PathType Leaf) {
        try {
            ...
            if ($null -ne $setupPluginAuthInfoBytes) {
                $setupPluginAuthInfoHeaderAsciiPrefix = & $getHeaderAsciiPrefix $setupPluginAuthInfoBytes
            }

            if (($null -ne $setupPluginAuthInfoBytes) -and $setupPluginAuthInfoBytes.Length -gt 0) {
    ...
    $directKdocerjsapi20PoolPackagePrefixPresent = $false
    $directKwpsaiwordtoolPoolPackagePrefixPresent = $false
    if (Test-Path -LiteralPath $addonPoolRoot -PathType Container) {
        $directKdocerjsapi20PoolPackagePrefixPresent = @(Get-ChildItem -LiteralPath $addonPoolRoot -Directory -Filter "kdocerjsapi20_*" -ErrorAction SilentlyContinue).Count -gt 0
        $directKwpsaiwordtoolPoolPackagePrefixPresent = @(Get-ChildItem -LiteralPath $addonPoolRoot -Directory -Filter "kwpsaiwordtool_*" -ErrorAction SilentlyContinue).Count -gt 0
    }
    ...
    $comAddinsCommandDatabaseSchemaSamples = [System.Collections.Generic.List[object]]::new()
    foreach ($databasePath in $addonPoolReconIndex.CommandDatabaseFiles) {
        try {
            $databaseBytes = [System.IO.File]::ReadAllBytes($databasePath)
            $databaseAsciiText = [System.Text.Encoding]::ASCII.GetString($databaseBytes)
            $containsComAddIns = $databaseAsciiText.Contains("COMAddIns")
            if ($containsComAddIns) {
                $comAddinsCommandDatabaseEvidence.Add($databasePath)
                $packageVersion = $null
                $hostDatabaseName = [System.IO.Path]::GetFileNameWithoutExtension($databasePath)
                $packageVersionMatch = [regex]::Match($databasePath, "\\kcmddb_([^\\]+)\\db\\personal_cn\\")
                ...
                if ($comAddinsCommandDatabaseSamples.Count -lt 8) {
                    $comAddinsCommandDatabaseSamples.Add([pscustomobject]@{
                        ...
                        ContainsKdocerjsapi20             = $databaseAsciiText.Contains("kdocerjsapi20")
                        ContainsKwpsaiwordtool            = $databaseAsciiText.Contains("kwpsaiwordtool")
                        ContainsPictureResourceshopSplit  = $databaseAsciiText.Contains("picture_resourceshop_split")
                        ContainsKdocerjsapi20EntryDll     = $databaseAsciiText.Contains("kdocerjsapi20.dll")
                        ContainsKdocerjsapi20EntryPoint   = $databaseAsciiText.Contains("CreateSplitAppWidget")
                        ContainsKdocerjsapi20LauncherType = $databaseAsciiText.Contains("proxyFrame")
                    })
                }

                if ($comAddinsCommandDatabaseSchemaSamples.Count -lt 4) {
                    try {
                        $headerAsciiPrefix = & $getHeaderAsciiPrefix $databaseBytes

                        $comAddinsCommandDatabaseSchemaSamples.Add([pscustomobject]@{
                            Path                          = $databasePath
                            PackageVersion                = $packageVersion
                            HostDatabaseName              = $hostDatabaseName
                            HeaderAsciiPrefix             = $headerAsciiPrefix
                            LooksLikeSqlite3              = ($headerAsciiPrefix -like "SQLite format 3*")
                            ContainsCreateTableCmdLiteral = $databaseAsciiText.Contains("CREATE TABLE cmd(")
                            ContainsSqliteMasterLiteral   = $databaseAsciiText.Contains("sqlite_master")
                            ContainsXmlColumnLiteral      = $databaseAsciiText.Contains("xml CHAR")
                            ContainsKProxyCommandLiteral  = $databaseAsciiText.Contains("KProxyCommand")
                            ContainsKDependsProxyCommandLiteral = $databaseAsciiText.Contains("KDependsProxyCommand")
                            ContainsKDocerPictureProxyCommandLiteral = $databaseAsciiText.Contains("KDocerPictureProxyCommand")
                            ContainsKxLegacyTriggerCommandLiteral = $databaseAsciiText.Contains("KxLegacyTriggerCommand")
                            ContainsTargetModuleKdocerjsapi20Literal = $databaseAsciiText.Contains("targetModule=""kdocerjsapi20""")
                            ContainsTargetModuleKwpsaiwordtoolLiteral = $databaseAsciiText.Contains("targetModule=""kwpsaiwordtool""")
                            ContainsTargetIdAttributeLiteral = $databaseAsciiText.Contains("targetId=""")
                            ContainsRelatedKsoCmdAttributeLiteral = $databaseAsciiText.Contains("relatedKsoCmd=""")
                            ContainsDependAppIdAttributeLiteral = $databaseAsciiText.Contains("dependAppId=""")
                            ContainsKsoCmdAttributeLiteral = $databaseAsciiText.Contains("ksoCmd=""")
                            ContainsProxyCommandsKuipLiteral = $databaseAsciiText.Contains("delayloadPath=""commands/wps/ongmani/ProxyCommands.kuip""")
                            ContainsComAddInsDialogIdMsoLiteral = $databaseAsciiText.Contains("idMso=""ComAddInsDialog""")
                            ContainsWordToolsContextLiteral = $databaseAsciiText.Contains("WordTools")
                            ContainsWpscommonContextLiteral = $databaseAsciiText.Contains("wpscommon")
                            ContainsPopupMenuContextLiteral = $databaseAsciiText.Contains("PopupMenu")
                            ContainsRelatedKsoCmdInsertPictureFromFileLiteral = $databaseAsciiText.Contains("relatedKsoCmd=""InsertPictureFromFile""")
                            ContainsDependAppIdKinsertpoLiteral = $databaseAsciiText.Contains("dependAppId=""kinsertpo""")
                            ContainsKsocmdComAddInsLiteral = $databaseAsciiText.Contains("ksoCmd=""COMAddIns""")
                            DirectKdocerjsapi20PoolPackagePrefixPresent = $directKdocerjsapi20PoolPackagePrefixPresent
                            DirectKwpsaiwordtoolPoolPackagePrefixPresent = $directKwpsaiwordtoolPoolPackagePrefixPresent
                        })
                    }
                    catch {
                    }
    ...
    foreach ($moduleFileName in @(
        ...
        $matchPath = if ($addonPoolReconIndex.SuspiciousModulePathsByName.ContainsKey($moduleFileName)) { [string]$addonPoolReconIndex.SuspiciousModulePathsByName[$moduleFileName] } else { $null }
        if ([string]::IsNullOrWhiteSpace($matchPath)) {
            continue
        }

        try {
            $moduleText = [System.Text.Encoding]::ASCII.GetString([System.IO.File]::ReadAllBytes($matchPath))
            $suspiciousModuleFiles.Add($matchPath)
            $namedExportSample = @((Get-NamedDllExports -Path $matchPath) | Select-Object -First 12)
            $suspiciousModuleSamples.Add([pscustomobject]@{
                Path                   = $matchPath
                NamedExportCount       = @($namedExportSample).Count
                NamedExportSample      = @($namedExportSample)
                HasDllRegisterServer   = $moduleText.Contains("DllRegisterServer")
                HasDllGetClassObject   = $moduleText.Contains("DllGetClassObject")
                HasProgIdLiteral       = $moduleText.Contains("ProgID")
                HasClsidLiteral        = $moduleText.Contains("CLSID")
                HasIDispatchLiteral    = $moduleText.Contains("IDispatch")
                HasLoadBehaviorLiteral = $moduleText.Contains("LoadBehavior")
            })
        }
        catch {
        }
    ...
    foreach ($metadataFileName in @("plugin-provider.json", "runinfo.json", "config.json", "run.ini")) {
        foreach ($metadataPath in $addonPoolReconIndex.MetadataFilesByName[$metadataFileName]) {
            if (-not $nativeMetadataSearchFiles.Contains($metadataPath)) {
                $nativeMetadataSearchFiles.Add($metadataPath)
            }

            if ($nativePluginPackageMetadataFiles.Count -lt 12 -and -not $nativePluginPackageMetadataFiles.Contains($metadataPath)) {
                $nativePluginPackageMetadataFiles.Add($metadataPath)
            }
        }
    }
    $nativeMetadataTextByPath = @{}
    foreach ($metadataPath in $nativeMetadataSearchFiles) {
        try {
            $nativeMetadataTextByPath[$metadataPath] = [System.Text.Encoding]::ASCII.GetString([System.IO.File]::ReadAllBytes($metadataPath))
        }
        catch {
            $nativeMetadataTextByPath[$metadataPath] = $null
        }
    }

    $commandDatabaseMetadataCrossReferenceSamples = [System.Collections.Generic.List[object]]::new()
    foreach ($crossReferenceToken in @(
        "kinsertpo",
        "picture_resourceshop_split",
        "photo2pdfentry",
        "processonentry",
        "kaiwps",
        "kwpsaitablestyle",
        ...
        $matchingMetadataFiles = @(
            $nativeMetadataSearchFiles |
                Where-Object {
                    $metadataText = if ($nativeMetadataTextByPath.ContainsKey($_)) { [string]$nativeMetadataTextByPath[$_] } else { $null }
                    (-not [string]::IsNullOrEmpty($metadataText)) -and $metadataText.Contains($crossReferenceToken)
                }
        )
        $commandDatabaseMetadataCrossReferenceSamples.Add([pscustomobject]@{
            Token                   = $crossReferenceToken
            AppearsInPoolMetadata   = ($matchingMetadataFiles.Count -gt 0)
            MatchingMetadataFileCount = $matchingMetadataFiles.Count
            MatchingMetadataFileSample = @($matchingMetadataFiles | Select-Object -First 5)
        })
    }
    ...
    foreach ($runInfoPath in $addonPoolReconIndex.MetadataFilesByName["runinfo.json"]) {
        if ($runInfoManifestSamples.Count -ge 4) {
            break
        }

        try {
            $runInfoRoot = ConvertFrom-Json ([System.IO.File]::ReadAllText($runInfoPath))
            foreach ($runInfoApp in $runInfoRoot.PSObject.Properties) {
                $entryDll = [string]$runInfoApp.Value.entryDll
                if ([string]::IsNullOrWhiteSpace($entryDll)) {
                    $entryDll = [string]$runInfoApp.Value.launcherDll
                }

                $entryPoint = [string]$runInfoApp.Value.entryPoint
                $launcherType = [string]$runInfoApp.Value.launcherType
                if ([string]::IsNullOrWhiteSpace($entryDll) -or [string]::IsNullOrWhiteSpace($entryPoint) -or [string]::IsNullOrWhiteSpace($launcherType)) {
                    continue
                }

                $runInfoManifestSamples.Add([pscustomobject]@{
                    Path         = $runInfoPath
                    AppId        = [string]$runInfoApp.Name
                    EntryDll     = $entryDll
    ...
    foreach ($configPath in ($addonPoolReconIndex.MetadataFilesByName["config.json"] | Select-Object -First 4)) {
        try {
            $configRoot = ConvertFrom-Json ([System.IO.File]::ReadAllText($configPath))
    ...
    foreach ($runIniPath in ($addonPoolReconIndex.MetadataFilesByName["run.ini"] | Select-Object -First 4)) {
        try {
            $runIniText = [System.IO.File]::ReadAllText($runIniPath)
    ...
    foreach ($preloadPath in ($addonPoolReconIndex.MetadataFilesByName["preload-file.json"] | Select-Object -First 4)) {
        try {
            $preloadRoot = ConvertFrom-Json ([System.IO.File]::ReadAllText($preloadPath))
    ...
    foreach ($binaryMetadataPath in (
        @($addonPoolReconIndex.MetadataFilesByName["plugin-provider.json"]) +
        @($addonPoolReconIndex.MetadataFilesByName["__attr.plg"]) |
        Select-Object -First 8
    )) {
        try {
            $binaryMetadataBytes = [System.IO.File]::ReadAllBytes($binaryMetadataPath)
            if ($binaryMetadataBytes.Length -le 0) {
                continue
            }

            $looksLikeTextJson = ($binaryMetadataBytes[0] -eq 0x7B) -or ($binaryMetadataBytes[0] -eq 0x5B)
            $binaryMetadataName = [System.IO.Path]::GetFileName($binaryMetadataPath)
            if ((-not $looksLikeTextJson) -or $binaryMetadataName -eq "__attr.plg") {
    ...
    if (Test-Path -LiteralPath $addonPoolRoot -PathType Container) {
        foreach ($nativeDllPrefix in @("kdocerjsapi20", "kwpsaiwordtool")) {
            ...
            try {
                $nativeDllText = [System.Text.Encoding]::ASCII.GetString([System.IO.File]::ReadAllBytes($nativeDllPath))
                $namedDllExports = @(Get-NamedDllExports -Path $nativeDllPath)
                $nativeDllPackageSamples.Add([pscustomobject]@{
                        ...
                        NamedExportCount          = @($namedDllExports).Count
                        NamedExportSample         = @($namedDllExports | Select-Object -First 12)
                        HasDllRegisterServer      = $nativeDllText.Contains("DllRegisterServer")
                        HasDllUnregisterServer    = $nativeDllText.Contains("DllUnregisterServer")
                        HasDllGetClassObject      = $nativeDllText.Contains("DllGetClassObject")
                        HasProgIdLiteral          = $nativeDllText.Contains("ProgID")
                        HasClsidLiteral           = $nativeDllText.Contains("CLSID")
                        HasIDispatchLiteral       = $nativeDllText.Contains("IDispatch")
                        HasLoadBehaviorLiteral    = $nativeDllText.Contains("LoadBehavior")
                    })
            }
            catch {
            }
            ...
            $listV3ShardDirectorySamples.Add([pscustomobject]@{
                ...
                SampleShardIdLiteralDetectedInPoolMetadata = @(
                    $nativeMetadataSearchFiles |
                        Where-Object {
                            $metadataText = if ($nativeMetadataTextByPath.ContainsKey($_)) { [string]$nativeMetadataTextByPath[$_] } else { $null }
                            (-not [string]::IsNullOrEmpty($metadataText)) -and $metadataText.Contains($numericShardDirectory.Name)
                        }
                ).Count -gt 0
            })
        }
    ...
    return [pscustomobject]@{
        ...
        ComAddinsCommandDatabaseSchemaSamples = @($comAddinsCommandDatabaseSchemaSamples.ToArray())
        CommandDatabaseMetadataCrossReferenceSamples = @($commandDatabaseMetadataCrossReferenceSamples.ToArray())
        ...
        Conclusion             = "WPS host detected with plugin framework directories, setupplugin.plg, binary listV3 plugin indexes, native pool package metadata files such as plugin-provider.json and runinfo.json, internal COM typelib traces, SQLite-format command databases under kcmddb_*/db/personal_cn, and a populated external add-in candidate root at HKCU:\\Software\\Kingsoft\\Office\\WPS\\AddinsWl. The shared COM add-ins dialog appears to be routed through kshell.dll with KxCOMAddinsDlg mapped to TCOMAddinsDlg.UnicodeClass across WPS, WPP, and ET. Current binary evidence shows sampled WPS, WPP, and ET application surfaces all exposing Application.COMAddIns alongside Application.AddIns. setupplugin.plg currently exposes host/mode/name/type plugin declarations, sampled runinfo.json files expose entryDll, entryPoint, and launcherType fields, sampled config.json files expose office_type routing, sampled run.ini files point to %workingroot%/index.html, and pluginlist.plg plus shard ids/data files all currently present as kplugin-headed binary payloads. Sampled command databases currently look like SQLite format 3 stores with a cmd table that includes text, ksocmd, context, and xml columns. They still expose COMAddIns, plugin-name literals such as kdocerjsapi20 and kwpsaiwordtool, and the runinfo-style app identifier picture_resourceshop_split, but do not expose entryDll, entryPoint, or launcherType literals such as kdocerjsapi20.dll, CreateSplitAppWidget, or proxyFrame. Current pool-root naming also appears drift-prone: command databases still reference targetModule kdocerjsapi20 even when the live pool root no longer exposes a direct kdocerjsapi20_* package directory, while kwpsaiwordtool still resolves directly. Sampled native DLL packages currently do not expose DllRegisterServer, DllGetClassObject, ProgID, CLSID, IDispatch, or LoadBehavior literals, and the sampled kdocerjsapi20 package instead exports JS/API-style entry names such as GetExtensionJsApiObj and loadJsapiService. On this machine, 78 AddinsWl entry names were observed, zero resolved to live ProgID roots, and only two carried non-empty value payloads that currently look version-gate-like rather than LoadBehavior-style flags. The stronger native package evidence indicates that WPS activation is mediated by its own plugin/index/runtime stack, with command databases representing a host-scoped menu or command layer rather than a direct mirror of native package runinfo manifests, so WordTools write semantics and activation behavior for AddinsWl therefore remain unvalidated."
    }
}
...
function Get-AddInRegistryPath {
    param(
        ...
        [string]$HostBitness = "x64"
    )

    switch ($HostName) {
        "Word" {
            if ($HostBitness -eq "x86") {
                return "HKLM:\Software\WOW6432Node\Microsoft\Office\Word\Addins\WordTools.ThisAddIn"
            }
            return "HKLM:\Software\Microsoft\Office\Word\Addins\WordTools.ThisAddIn"
        }
...
function Test-LiveExecutionAllowedForHost {
    ...
    $registryPath = Get-AddInRegistryPath -HostName $HostTarget.HostName -HostBitness $HostTarget.HostBitness
...
function Get-DryRunRegistrationPlan {
    ...
    foreach ($detectedHost in $requestedSelection.Hosts) {
        ...
        $registryPath = Get-AddInRegistryPath -HostName $detectedHost.HostName -HostBitness $detectedHost.HostBitness
        ...
        $targets.Add([pscustomobject]@{
                ...
                DecisionReason   = if ($wouldRegister) {
                    "Supported host detected. Dry-run shows the future Word x64 registration path without executing it."
                }
                else {
...
function Get-PlanRegistrationPlan {
    param(
        [Parameter(Mandatory = $true)]
        [object]$ProbeResult
    )
    ...
    $registerPreviewPlan = Get-RegisterPreviewPlan -ProbeResult $ProbeResult

    return [pscustomobject]@{
        ExecutionMode           = $dryRunPlan.ExecutionMode
        PlanSummary             = $dryRunPlan.PlanSummary
        Targets                 = $dryRunPlan.Targets
        LiveReadinessSummary    = $registerPreviewPlan.LiveReadinessSummary
        InstallerHandoffSummary = $registerPreviewPlan.InstallerHandoffSummary
        MigrationChecklist      = $registerPreviewPlan.MigrationChecklist
        InstallerPreviewReport  = $registerPreviewPlan.InstallerPreviewReport
    }
}
...
function Get-LiveReadinessSummary {
    ...
    $experimentalHostLabels = [System.Collections.Generic.List[string]]::new()
    ...
    foreach ($summary in $HostRuleSummaries) {
        ...
        switch ($summary.CurrentSupportStatus) {
            ...
            "experimental" { $experimentalHostLabels.Add($hostLabel) }
    ...
    $overallDecision = if ($liveReadyHostLabels.Count -gt 0) {
        if ($experimentalHostLabels.Count -gt 0) {
            "Live readiness has $($liveReadyHostLabels.Count) host ready for live execution, $($previewOnlyHostLabels.Count) supported host still gated behind preview-only enforcement, and $($experimentalHostLabels.Count) experimental host still waiting on formal validation."
        }
        else {
    ...
    elseif ($previewOnlyHostLabels.Count -gt 0) {
        if ($experimentalHostLabels.Count -gt 0) {
            "All currently supported detected hosts remain preview-only, and experimental hosts are still waiting on formal validation before they can become live-eligible."
        }
        else {
            "All currently supported detected hosts remain preview-only; live execution is still disabled in the shared installer core."
        }
    }
    elseif ($experimentalHostLabels.Count -gt 0) {
        if ($probePendingHostLabels.Count -gt 0) {
            "Detected hosts include experimental and probe-pending targets. Experimental hosts still need formal validation, and planned hosts still need probe completion before they can advance to live execution."
        }
        else {
            "Detected hosts are currently experimental and still need formal validation before they can advance to live execution."
        }
    }
    ...
    return [pscustomobject]@{
        ...
        ExperimentalHostCount  = $experimentalHostLabels.Count
        ...
        ExperimentalHostLabels = @($experimentalHostLabels.ToArray())
...
function Get-InstallerHandoffSummary {
    ...
    $experimentalHosts = [System.Collections.Generic.List[string]]::new()
    ...
    foreach ($detectedHost in $ProbeResult.Hosts) {
        ...
        switch ([string]$detectedHost.SupportStatus) {
            "supported" {
                $supportedHosts.Add($hostLabel)
            }
            "experimental" {
                $experimentalHosts.Add($hostLabel)
            }
            default {
    ...
    $decision = if ($liveCapableHosts.Count -gt 0) {
        if ($experimentalHosts.Count -gt 0) {
            "Supported hosts are available for shared-core live registration through the rerouted installer and script entrypoints. Experimental hosts remain preview-only until formal P0 validation completes."
        }
        else {
            "Supported hosts are available for shared-core live registration through the rerouted installer and script entrypoints."
        }
    }
    elseif ($supportedHosts.Count -gt 0) {
        if ($experimentalHosts.Count -gt 0) {
            "Detected supported hosts remain preview-only, and experimental hosts also remain outside live execution until formal validation completes."
        }
        else {
            "Detected supported hosts remain preview-only and are not currently live-routable."
        }
    }
    elseif ($experimentalHosts.Count -gt 0) {
        if ($unsupportedHosts.Count -gt 0) {
            "Only experimental or planned hosts were detected. They remain preview-only until formal support validation completes."
        }
        else {
            "Only experimental hosts were detected. They remain preview-only until formal P0 validation completes."
        }
    }
    ...
    return [pscustomobject]@{
        ...
        ExperimentalHostCount = $experimentalHosts.Count
        ...
        ExperimentalHosts   = @($experimentalHosts.ToArray())
...
function Get-MigrationChecklist {
    ...
    foreach ($target in $ProbeResult.SupportState.targets) {
        if ($target.status -eq "planned" -or $target.status -eq "experimental") {
    ...
    $overallDecision = if ($readyToReroute) {
        "Live entrypoint reroute is complete for currently supported hosts. Remaining experimental and planned targets still require formal validation before support expansion."
    } else {
...
function Get-InstallerPreviewReport {
    ...
    $experimentalHosts = @($InstallerHandoffSummary.ExperimentalHosts)
    $supportedHostText = if ($supportedHosts.Count -gt 0) { $supportedHosts -join ", " } else { "none" }
    $experimentalHostText = if ($experimentalHosts.Count -gt 0) { $experimentalHosts -join ", " } else { "none" }
    ...
    return [pscustomobject]@{
        ...
        ExperimentalHostCount        = $InstallerHandoffSummary.ExperimentalHostCount
        UnsupportedHostCount         = $InstallerHandoffSummary.UnsupportedHostCount
        ReadyToRerouteLiveEntrypoints = $MigrationChecklist.ReadyToRerouteLiveEntrypoints
        SummaryText                  = "Detected hosts: $($ProbeResult.Hosts.Count). Supported hosts: $supportedHostText. Experimental hosts: $experimentalHostText. Preview actions: $($OperationManifest.Count)."
    }
}
...
function Get-RegisterPreviewTargetForWordHost {
    ...
    return [pscustomobject]@{
        ...
        DecisionReason  = if ($wouldExecute) {
            "Preview-only register mode would execute the current live-enabled registration path, but live registration remains disabled in the shared core."
        }
        else {
...
function Get-UnregisterPreviewTargetForWordHost {
    ...
    return [pscustomobject]@{
        ...
        DecisionReason  = if ($wouldExecute) {
            "Preview-only unregister mode would remove the supported Word x64 registration path, but live unregistration remains disabled in the shared core."
        }
        else {
...
function New-SkippedNgenResult {
    param(
        [Parameter(Mandatory = $true)]
        [string]$HostBitness,
    ...
    return [pscustomobject]@{
        ExecutionMode = "Live"
        ToolPath      = Get-NgenPath -HostBitness $HostBitness
        Attempted     = $false
        Succeeded     = $false
        Action        = "Install"
...
function Get-HostRuleEnablementCondition {
    ...
    if ($HostTarget.HostName -eq "Word") {
        if ($LiveExecutionAllowed) {
            return "Live $Operation for Word is enabled when the host remains supported and both the RegAsm path and Word add-in registry target resolve successfully. The shared core can execute live Word registration for the current live-enabled Word path when invoked through a live entrypoint."
        }
...
function Register-WordHost {
    ...
    $registryTarget = Get-AddInRegistryPath -HostName $HostTarget.HostName -HostBitness $HostTarget.HostBitness
    ...
    Write-LiveProgress ("Registering {0} {1}: running RegAsm..." -f $HostTarget.HostName, $HostTarget.HostBitness)
    ...
    Write-LiveProgress ("Registering {0} {1}: writing COM add-in registry..." -f $HostTarget.HostName, $HostTarget.HostBitness)
    $registryResult = Set-WordAddInRegistry -RegistryPath $registryTarget
    $ngenResult = if ($RunNgen) {
        Write-LiveProgress ("Registering {0} {1}: running NGen precompilation..." -f $HostTarget.HostName, $HostTarget.HostBitness)
        Invoke-NgenForHost -DllPath $DllPath -HostBitness $HostTarget.HostBitness
    }
    else {
        Write-LiveProgress ("Registering {0} {1}: skipping NGen for faster completion." -f $HostTarget.HostName, $HostTarget.HostBitness)
        New-SkippedNgenResult -HostBitness $HostTarget.HostBitness -Reason "NGen was skipped by default for faster registration. Re-run with -RunNgen to enable precompilation."
    }
...
function Get-SupportSummary {
    ...
    foreach ($detectedHost in $Hosts) {
        ...
        switch ($detectedHost.SupportStatus) {
            ...
            "experimental" { $experimentalHosts.Add($hostLabel) }
    ...
    return [pscustomobject]@{
        ...
        ExperimentalHosts    = $experimentalHosts
...
function Get-HostInventory {
    ...
    $allCandidates = @()
    $probeHosts = if ($Mode -in @("Plan", "Register", "Unregister")) {
        Get-RequestedHostNames -RequestedHost $RequestedHost
    }
    else {
        @("Word", "WPS")
    }

    if ($probeHosts -contains "Word") {
        Write-LiveProgress "Inspecting installed Word hosts..."
        $allCandidates += Get-InstalledWordCandidates
    }

    if ($probeHosts -contains "WPS") {
        Write-LiveProgress "Inspecting installed WPS hosts..."
...
function Get-InstallerDecisionSummaryText {
    ...
    $experimentalHostsText = if ($experimentalHosts.Count -gt 0) { $experimentalHosts -join ", " } else { "(none)" }
    ...
    $lines = @(
        ...
        ("ExperimentalHosts: {0}" -f $experimentalHostsText),
...
function Invoke-LiveRegisterExecution {
    ...
    Write-LiveProgress "Resolving live registration inputs..."
    ...
    if ($selection.EligibleHosts.Count -eq 0) {
        throw "No live-registration-eligible host matched the current request. Only hosts that are currently live-enabled by the support matrix and host rule gate are allowed."
    }

    Assert-LiveExecutionAdministrator -Operation "Register"
    Write-LiveProgress ("Live registration will apply to {0} host(s)." -f $selection.EligibleHosts.Count)
    ...
    foreach ($eligibleHost in $selection.EligibleHosts) {
        switch ($eligibleHost.HostName) {
            "Word" {
                $targetResults.Add((Register-WordHost -HostTarget $eligibleHost -DllPath $dllPath -RunNgen:$RunNgen))
            }
            "WPS" {
                $targetResults.Add((Register-WpsHost -HostTarget $eligibleHost -DllPath $dllPath -RunNgen:$RunNgen))
            }
            default {
...
function Invoke-LiveUnregisterExecution {
    ...
    if ($selection.EligibleHosts.Count -eq 0) {
        throw "No live-unregistration-eligible host matched the current request. Only hosts that are currently live-enabled by the support matrix and host rule gate are allowed."
    }
...
switch ($Mode) {
    ...
    "Plan" {
        ...
        $planResult = [pscustomobject]@{
            ...
            ProbedAtUtc      = $probeResult.ProbedAtUtc
            SupportState     = $probeResult.SupportState
            SupportSummary   = $requestedProbeResult.SupportSummary
            Hosts            = $requestedProbeResult.Hosts
            RegistrationPlan = Get-PlanRegistrationPlan -ProbeResult $requestedProbeResult
        }
        ...
        switch ($Action) {
            ...
            "write" {
                if ([string]::IsNullOrEmpty($ProgId)) {
                    throw "ProgId cannot be empty for write action."
                }
